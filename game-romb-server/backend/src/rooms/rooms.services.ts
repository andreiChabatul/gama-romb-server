...
export class RoomsService {
  ...
  controlRoom(
    { action, idUser, idRoom, colorPlayer }: ControlRoomPayload,
    client: myWebSocket,
  ): void {
    switch (action) {
      case 'join':
  ...
  async getAllRooms(): Promise<infoRoom[]> {
    ...
    return await Promise.all(
      Object.keys(this.rooms).map((key) => this.rooms[key].returnInfoRoom()),
    );
  }
    ...
    if (room && idUser) {
      if (room.stateRoom) {
        const timer = setTimeout(() => {
          ...
          this.playersOffline = this.playersOffline.filter(
            (player) => player && player.idUser !== idUser,
          );
        }, TIME_DISCONNECT);
  ...
  async reconnectPlayerId(idUser: string): Promise<infoRoom | undefined> {
    const player = this.playersOffline.filter(
      (player) => player && player.idUser === idUser,
    )[0];
