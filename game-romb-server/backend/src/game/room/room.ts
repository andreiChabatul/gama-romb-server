...
import {
  ContorolCompanyPayload,
  ControlAuctionPayload,
  DiceRollGamePayload,
  EACTION_WEBSOCKET,
...
export class RoomGame implements RoomI {
  ...
  addPlayer(idUser: string, color: string): void {
    const player = new PlayerDefault(
      idUser,
      this.idRoom,
      color,
      this.cellsService,
  ...
  activeCell(idUser: string): void {
    ...
    if (player.turn || player.prison)
      this.cellsService.activateCell(player.position, idUser);
    ...
    switch (action) {
      ...
      case 'refuse':
        chatGame.addChatMessage(this.idRoom, {
          action: EMESSAGE_CLIENT.REFUSE_DEAL,
          idUser,
        });
      ...
      case 'accept':
        chatGame.addChatMessage(this.idRoom, {
          action: EMESSAGE_CLIENT.ACCEPT_DEAL,
          idUser,
        });
