...
export class AuctionCompany implements AuctionI {
  ...
  private filterParticipants(idUser?: string): void {
    ...
    this.playersId = this.playersId.filter(
      (id) =>
        id !== idUser &&
        storage_players.getPlayer(this.idRoom, id).total >= futureCurrentPrice,
    );
    this.nextStep();
  }

  private nextStep(): void {
    (this.playersId.length === 1 && this.auctionWinner) ||
    this.playersId.length === 0
      ? this.endAuction()
      : this.indexActive > this.playersId.length - 2
      ? 0
      : (this.indexActive += 1);
    this.sendAllPlayers();
  }
  ...
  private sendAllPlayers(): void {
    storage_players.getPlayersActive(this.idRoom).map((idUser) => {
      let desc: statePlayer = this.playersId.includes(idUser)
        ? 'wait'
        : 'inactive';
