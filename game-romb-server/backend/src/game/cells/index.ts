...
export class CellsService implements CellsServiceI {
  ...
  constructor(idRoom: string) {
    this.cells = defaultCell.map((cell, indexCell) => {
      switch (cell.type) {
        case 'company':
          return new CellCompany(
            indexCell,
            idRoom,
            cell.company,
            this.updateMonopoly.bind(this),
          );
        case 'empty':
    ...
    return this.cells.reduce(
      (capital, cell) =>
        'owned' in cell && cell.owned === idUser && !cell.pledge
          ? capital +
            cell.infoCompany.collateralCompany +
            cell.quantityStock * cell.infoCompany.priceStock
          : capital,
      0,
    );
  }
  ...
  updateMonopoly(countryCompany: countryCompany): void {
    const cells = this.cells.filter(
      (cell) =>
        'owned' in cell && cell.infoCompany.countryCompany === countryCompany,
    ) as CellCompanyI[];
    if (
      cells[0].infoCompany.countryCompany !== 'japan' &&
      cells[0].infoCompany.countryCompany !== 'ukraine'
    ) {
      const ownedCell = cells.map((cell) =>
        cell.owned && !cell.pledge ? cell.owned : 'noOwned',
      );
      const resultArr = [...new Set(ownedCell)];
      cells.forEach(
        (cell) =>
          (cell.monopoly =
            resultArr.length === 1 && !resultArr.includes('noOwned')),
      );
    } else {
