...
export class TurnService {
  ...
  firstTurn(): void {
    ...
    chatGame.addChatMessage(this.idRoom, {
      action: EMESSAGE_CLIENT.FIRST_TURN,
      idUser: this.activePlayer,
    });
    this.updateTurn();
  }
  ...
  turn(idUser: string, valueroll: number, isDouble: boolean): void {
    if (this.checkPrisonUser(idUser, isDouble)) {
      chatGame.addChatMessage(this.idRoom, {
        action: EMESSAGE_CLIENT.DOUBLE_TURN_PRISON,
        idUser,
      });
  ...
  private nextTurn(): void {
    if (this.isDouble) {
      chatGame.addChatMessage(this.idRoom, {
        action: EMESSAGE_CLIENT.DOUBLE_TURN,
        idUser: this.activePlayer,
      });
    } else {
      this.indexActive = this.calcIndexActive();
      chatGame.addChatMessage(this.idRoom, {
        action: EMESSAGE_CLIENT.TURN,
        idUser: this.activePlayer,
      });
    }
    this.isDouble = false;
  ...
  private calcIndexActive(): number {
    ...
    futureIndexActive >= this.playersActive.length
      ? (futureIndexActive = 0)
      : '';
