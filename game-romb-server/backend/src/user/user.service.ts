...
export class UserService {
  ...
  async updateUser(id: string, updateUserDto: UpdateUserDto): Promise<User> {
    ...
    if (userRepeat && userRepeat.id !== user.id) {
      throw new BadRequestException(
        'Ошибка обновления. Пользователь с новым никнеймом существует.',
      );
    }
      ...
      data: {
        nickName:
          user.nickName === updateUserDto.newNickName
            ? undefined
            : updateUserDto.newNickName,
        image:
          user.image === updateUserDto.newAvatar
            ? undefined
            : updateUserDto.newAvatar,
        password:
          updateUserDto.newPassword === '*******'
            ? undefined
            : this.hashPassword(updateUserDto.newPassword),
      },
    });
    ...
    throw new BadRequestException(
      'Пользователя не существует или пароль неверный',
    );
  }
  ...
  async deleteUser(deleteUserDto: DeleteUserDto): Promise<string> {
    ...
    if (
      user &&
      user.nickName === deleteUserDto.nickname &&
      compareSync(deleteUserDto.password, user.password)
    ) {
